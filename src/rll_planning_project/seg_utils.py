...
def s2s_ixt(s0, s1, tol=1e-3, atol=3e-2, as_pt=False):
    (a,b), (c,d) = s0, s1
    v0 = (b-a)
    v1 = (d-c)
    ...
    B = (c-a)
    t = np.linalg.solve(A, B) # parametric unit
    ...
    ix_check = np.any([dt < tol, dt_a < atol])
...
def ixtest():
    ...
    seg_1 = [[-0.17337501,  0.44550782],
            [-0.17337501,  0.565     ]]
    seg_1 = np.asarray(seg_1)
    seg_2 = [[-0.36549023,  0.43550783],
            [ 0.3667334,   0.43550783]]
    seg_2 = np.asarray(seg_2)
    ix = s2s_ixt(seg_1,seg_2,tol=5e-2,as_pt=True)
    
    plt.clf()
    plt.plot(seg_1[:,0], seg_1[:,1], 'r-')
    plt.plot(seg_2[:,0], seg_2[:,1], 'b-')
    if ix is not None:
...
def pstest():
    ...
    segs =[
            [[-0.37200001,  0.375     ], [-0.37200001,  0.56187499]],
            [[-0.48337501,  0.565     ], [ 0.35657811,  0.565     ]],
            [[-0.17337501,  0.44550782], [-0.17337501,  0.565     ]],
            [[-0.36549023,  0.43550783], [ 0.3667334 ,  0.43550783]],
            [[ 0.        , -0.56187499], [ 0.        , -0.44546875]],
            [[-0.36515623, -0.57187498], [ 0.34734374, -0.57187498]],
            [[-0.36515623, -0.43187499], [ 0.36515623, -0.43187499]],
            [[-0.37515622, -0.71068358], [-0.37515622, -0.36821043]],
            [[-0.37515622, -0.71294922], [-0.37515622, -0.37577146]],
            [[ 0.36484376, -0.56200194], [ 0.36484376,  0.55928707]],
            [[-0.48475584, -0.72068357], [ 0.13434204, -0.72068357]],
            [[-0.48475584, -0.72294921], [ 0.13434204, -0.72294921]],
            [[ 0.24798828, -0.01200194], [ 0.36484376, -0.01200194]],
            [[-0.36550292,  0.42799807], [ 0.36484376,  0.42799807]],
            ]
